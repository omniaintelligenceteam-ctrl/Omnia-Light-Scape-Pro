...
export const Home: React.FC = () => {
  // Use Neon Auth for basic status if available, but keep existing app auth for local logic for now
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden w-full pb-16 md:pb-20">
        <header className="px-6 py-4 md:py-4 md:px-10 flex items-center justify-between md:justify-between justify-between bg-[#111] text-white shadow-sm z-20 shrink-0 border-b border-gray-800">
          ...
          <div className="flex items-center gap-4">
             {/* Neon Auth Account Link */}
             {neonUser ? (
                <Link to="/account" className="hidden md:flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#F6B45A] hover:text-white transition-colors">
                    <span>Account ({neonUser.email.split('@')[0]})</span>
                </Link>
             ) : (
                <Link to="/auth" className="hidden md:flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors">
                    Login
                </Link>
             )}
