...
// Auth component removed, we use the Router protection now
...
import { useAuth } from '../lib/auth'; // Import Neon Auth hook

export const Dashboard: React.FC = () => {
  // --- AUTH INTEGRATION ---
  const { user: neonUser, logout } = useAuth();
  
  // Local User State (Mapped from Neon User)
  const [user, setUser] = useState<User | null>(null);

  // Sync Neon User to Local App User Structure
  useEffect(() => {
    if (neonUser) {
        setUser({
            id: neonUser.id,
