...
export const Chatbot: React.FC<ChatbotProps> = ({ currentView }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Prepare history for API (excluding welcome message if needed, but Gemini handles it fine)
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-24 right-4 md:bottom-24 md:right-8 z-50 w-12 h-12 md:w-14 md:h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-105 ${
          isOpen ? 'bg-[#111] text-white rotate-90' : 'bg-[#111] text-[#F6B45A] border border-[#F6B45A]/20'
        }`}
      >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-40 right-4 md:bottom-44 md:right-8 z-50 w-[90vw] md:w-[400px] h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 zoom-in-95 duration-300 font-sans">
